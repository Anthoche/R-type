...
namespace game::parsing {
	static void check_field(const nlohmann::json &data, const std::string &field, nlohmann::json::value_t expected_type) {
		...
		if (!data.at(field).is_number() && !data.at(field).is_string() && !data.at(field).is_boolean()) {
	...
	ecs::entity_t parse_player(ecs::registry &reg, const nlohmann::json &player_data) {
		try {
			...
			std::cout << "[DEBUG] Parsed player at (" << x << ", " << y << ")" << std::endl;
			return game::entities::create_player(reg, x, y, image_path);
	...
	ecs::entity_t parse_text(ecs::registry &reg, const nlohmann::json &text_data) {
		try {
			...
			return game::entities::create_text(reg, Vector2{x, y}, content, WHITE, font_size);
		} catch (const std::exception &e) {
