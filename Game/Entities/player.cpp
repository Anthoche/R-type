...
ecs::entity_t create_player(ecs::registry &reg, float x, float y) {
	...
	drawable.r = 0.f; drawable.g = 1.f; drawable.b = 1.f; drawable.a = 1.f; // Cyan
	...
	reg.add_system<component::velocity, component::controllable>(
		[](ecs::registry &reg,
		   ...
		   ecs::sparse_array<component::controllable> &ctrl) {
			...
			// Pour l'instant, on laisse la vitesse inchangée
			// Le système d'input sera géré par la couche de rendu
		});
}
